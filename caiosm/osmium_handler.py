...
class CaiRoutesHandler(osmium.SimpleHandler):
    ...
    def __init__(self, separator=",", infomont=False, debug=False):
        ...
        self.memb_schema = {
            "geometry": "LineString",
            "properties": OrderedDict([("IDPerc", "str"), ("IDtrat", "str")]),
        }
        if self.infomont:
            self.route_schema["properties"] = OrderedDict(
                [
                    ("IDPerc", "str"),
                    ("Nume", "str"),
                    ("Denomi", "str"),
            ...
            self.way_schema["properties"] = OrderedDict(
                [
                    ("osm_id_way", "int"),
                    ("TIPOLOGIA", "str"),
                    ("CARATTER", "str"),
    ...
    def length(self, epsg="EPSG:3035", unit="m"):
        ...
        """
        project = partial(
            pyproj.transform, pyproj.Proj(init="EPSG:4326"), pyproj.Proj(init=epsg)
        )
        alreadid = []
        # for each route
        total = 0
    ...
    def create_routes_geojson(self):
        ...
        for k, v in self.routes.items():
            ...
            if self.infomont:
                ...
                if (
                    "symbol" in tagskey
                    or "symbol:it" in tagskey
                    or "osmc:symbol" in tagskey
                ):
                    # check the tags in order
                    if "osmc:symbol" in tagskey:
                        if (
                            "red:red:white_stripe" in tags["osmc:symbol"]
                            or "red:red:white_bar" in tags["osmc:symbol"]
                        ):
                        ...
                        if "non segnalato" in tags["symbol:it"]:
                            ...
                            "su bandierina bianca e rossa" in tags["symbol:it"]
                ...
                outags = tags
            feats.append(geojson.Feature(geometry=mapping(geom), properties=outags))
    ...
    def write_geojson(self, out, typ="route", driv="GeoJSON", enc="utf-8", epsg=4326):
        ...
        if typ == "route":
            ...
            raise ValueError(
                "Accepted value for typ option are: 'route'," " 'way' and 'members'"
            )
        if self.debug:
            print(schema)
        project = None
    ...
    def write_relation_members_infomont(self, out):
        ...
        for feat in self.wjson["features"]:
            ...
            for w in self.members[osmid]:
                mystr = "{rel}{sep}{way}\n".format(rel=w, sep=self.sep, way=newid)
                outext += mystr
