...
class CaiOsmPg:
    ...
    def __init__(
        ...
        self.regions = None
        self.cai_where = (
            "route='hiking' and (tags ? 'cai_scale' or operator"
    ...
    def set_administrative_bounds(
        self, adminlevel="4", others="tags ? 'ISO3166-2' and tags ? 'ref:ISTAT'"
    ):
        ...
        """
        sql = (
            "select name as reg, st_union(way) as poly from {pre}_polygon "
            "where admin_level='{lev}' and boundary='administrative'".format(
                pre=self.prefix, lev=adminlevel
            )
        )
        if others:
    ...
    def route_count_lenght(self):
        ...
        if self.regsql:
            sql = (
                "select reg, count(osm_id), round(cast(sum(st_length("
                ...
                "poly) group by reg order by reg".format(
                    reg=self.regsql, pre=self.prefix, cai=self.cai_where
                )
            )
        else:
            sql = (
                "select count(osm_id), sum(st_length(way))/1000 as km_cai "
                "from {pre}_line where {cai}".format(
                    pre=self.prefix, cai=self.cai_where
                )
            )
    ...
    def not_route_lenght(self, highways=None):
        ...
        if self.regsql:
            sql = (
                "select reg, round(cast(sum(st_length(ST_Intersection(way,"
            ...
            sql += (
                " and not ST_Within(l.way, r.way)) as"
                " nocai where ST_intersects(nocai.way, poly) group by reg"
                " order by reg;"
            )
            sql = sql.format(reg=self.regsql, pre=self.prefix, cai=self.cai_where)
        else:
            sql = (
                "select sum(st_length(l.way))/1000 as km_nocai from "
                "{pre}_line as l, (select way from planet_osm_line where "
                "{cai}) as r where l.highway is not null and not "
                "ST_Within(l.way, r.way);".format(pre=self.prefix, cai=self.cai_where)
            )
        return self._execute(sql)
    ...
    def print_italy(self, fname="cai_map_italy.png"):
        ...
        paths = gpd.read_postgis(
            "select * from planet_osm_line where {}".format(self.cai_where),
            self.conn,
            geom_col="way",
            crs="epsg:32632",
        )
        region = gpd.read_postgis(
            self.regsql, self.conn, geom_col="poly", crs="epsg:32632"
        )
    ...
    def print_region(self, region):
        ...
        paths = gpd.read_postgis(
            "select distinct cai.* from (select name as reg, st_union(way)"
            " as poly from planet_osm_polygon where admin_level='4' and "
            "boundary='administrative' and tags ? 'ISO3166-2' and tags ? "
            "'ref:ISTAT' and name='{REG}' group by name) as lig, (select * "
            "from planet_osm_line where {CAIWHERE}) as cai where"
            " ST_intersects(way, poly)".format(REG=region, CAIWHERE=self.cai_where),
            self.conn,
            geom_col="way",
            crs="epsg:32632",
        )
        region = gpd.read_postgis(
            "select name as reg, st_union(way) as poly from planet_osm_polygon"
            " where admin_level='4' and boundary='administrative' and "
            "tags ? 'ISO3166-2' and tags ? 'ref:ISTAT' and name='{REG}'"
            " group by name".format(REG=region),
            self.conn,
            geom_col="poly",
            crs="epsg:32632",
        )
